<?php
// public/customer/customers_edit.php
...
require_once dirname(__DIR__, 2) . '/classes/customer.class.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_customer'])) {
    ...
    $message = $customer->update_customer($customer_id, $address, $phone, $email);

    // PRG: if ok, go back to list with message
    if (strpos($message, 'failed') === false) {
        header('Location: index.php?page=home&msg=' . urlencode($message));
        exit;
    }
}
...
$row  = null;
$data = $customer->read_customer($customer_id); // returns fetch(PDO::FETCH_ASSOC)
...
if (is_array($data) && isset($data['customer_id'])) {
    ...
    $message = $data; // error string or false
}
?>
<!DOCTYPE html>
<html>
...
<body>
    ...
    <a href="profile.php" class="back-link">&laquo; Back to profile</a>
