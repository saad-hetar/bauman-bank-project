...
// ---------- LOAD customer customer RECORD ----------
$emp_result = $customer->read_customer($customerId); // returns fetchAll()
...
// if ($emp) {
    ...
    if ($passport_id !== null) {
        $passport_result = $customer->read_passport($passport_id);
    ...
    if ($login_id !== null) {
        $login_result = $customer->read_login($login_id);
        $login = $login_result;
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_emp'])) {
    ...
    // call the customer wrapper which uses update_emp stored procedure
    $message = $customer->update_customer($customerId, $branch_id, $email, $phone, $lore, $quit_date);

    // reload emp/login/passport after update
    $emp_result = $customer->read_customer($customerId);
    if (is_array($emp_result) && count($emp_result) > 0) {
        $emp = $emp_result[0];
        $passport_id = $emp['passport_id'] ?? null;
...
$_SESSION['customer_id'] = $emp['customer_id'] ?? '';
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_pass'])) {
    ...
    if ($new_pass === "" || $confirm === "") {
        ...
        if ($login_id) {
            ...
            $pass_msg = $customer->update_passwort($login_id, $new_pass);
            ...
            $pass_msg = "Cannot update password: no login record found for this customer.";
        }
    }
}
?>
...
<head>
    <meta charset="utf-8">
    <title>customer profile – Bauman Bank</title>
    <style>
    body {
...
<body>
    ...
    <div class="box">
        ...
        <?php if ($emp): ?>
        <p><strong>customer ID:</strong> <?= htmlspecialchars($emp['customer_id']) ?></p>
        <p><strong>Email:</strong> <?= htmlspecialchars($emp['email']) ?></p>
        <p><strong>Phone:</strong> <?= htmlspecialchars($emp['phone']) ?></p>
        <p><strong>address:</strong> <?= htmlspecialchars($emp['address']) ?></p>
        <p><strong>created at:</strong> <?= htmlspecialchars($emp['created_at']) ?></p>

        <a href="customers_edit.php?customer_id=<?= htmlspecialchars($emp['customer_id']) ?>" class="button">Edit
            customer
            info</a>
