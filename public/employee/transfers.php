...
$employee = new employee($_SESSION['user_id'] ?? 0);
echo $employee->last_transfer_id;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['internal_card'])) {
        $message = $employee->transfer_internal_card(
    ...
    if (isset($_POST['external_card'])) {
        $message = $employee->transfer_external_card(
            $_POST['sender_card_num'] ?? '',
            $trans_type,
            $currency,
            $description,
    ...
    if (isset($_POST['internal_phone'])) {
        $message = $employee->transfer_internal_phone(
            $_POST['sender_phone'] ?? '',
            $trans_type,
            $currency,
            $description,
    ...
    if (isset($_POST['external_phone'])) {
        $message = $employee->transfer_external_phone(
            $_POST['sender_phone'] ?? '',
            $trans_type,
            $currency,
            $description,
    ...
    $_SESSION['last_transfer_id'] = $employee->last_transfer_id;
...
// Search GET
$q = $_GET['q'] ?? '';
$rows = $q !== '' ? $employee->search_transfer($q) : $employee->read_all_transfer();

// Preload cards & phones for dropdowns
$all_cards = $employee->read_all_card();
$all_customers = $employee->read_all_customer();
$all_central_bank_customers = $employee->read_all_central_bank_customer();
