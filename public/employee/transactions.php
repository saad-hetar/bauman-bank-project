...
$employee = new employee($_SESSION['user_id'] ?? 0); // make sure $employee exists
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['deposit'])) {
        $message = $employee->deposit($card_num, $amount, $description);
    }

    if (isset($_POST['pay'])) {
        $trans_type = $_POST['trans_type'] ?? 'pay';
        $message = $employee->pay($card_num, $amount, $trans_type, $description);
    }

    if (isset($_POST['withdraw'])) {
        $message = $employee->withdraw($card_num, $amount, $description);
    }

    if (isset($_POST['cancel_payment'])) {
        $trans_id = $_POST['trans_id'] ?? '';
        $message  = $employee->cancel_payments($trans_id);
    }

    $_SESSION['transactions_message'] = $message;
    // header('Location: index.php?page=transactions');
    $_SESSION['last_transaction_id'] = $employee->last_transaction_id;
...
$q = $_GET['q'] ?? '';
if ($q !== '') {
    $rows = $employee->search_transaction($q);
} else {
    $rows = $employee->read_all_transaction();
}
