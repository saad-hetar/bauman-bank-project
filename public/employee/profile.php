...
// ---------- LOAD employee EMPLOYEE RECORD ----------
$emp_result = $employee->read_emp($employeeId); // returns fetchAll()
...
// if ($emp) {
    ...
    if ($passport_id !== null) {
        $passport_result = $employee->read_passport($passport_id);
    ...
    if ($login_id !== null) {
        $login_result = $employee->read_login($login_id);
        $login = $login_result;
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_emp'])) {
    ...
    // call the employee wrapper which uses update_emp stored procedure
    $message = $employee->update_emp($employeeId, $branch_id, $email, $phone, $lore, $quit_date);

    // reload emp/login/passport after update
    $emp_result = $employee->read_emp($employeeId);
    if (is_array($emp_result) && count($emp_result) > 0) {
        $emp = $emp_result[0];
        $passport_id = $emp['passport_id'] ?? null;
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_pass'])) {
    ...
    if ($new_pass === "" || $confirm === "") {
        ...
        if ($login_id) {
            ...
            $pass_msg = $employee->update_passwort($login_id, $new_pass);
            ...
            $pass_msg = "Cannot update password: no login record found for this employee.";
        }
    }
}
?>
...
<head>
    <meta charset="utf-8">
    <title>employee profile – Bauman Bank</title>
    <style>
    body {
