...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['create_customer'])) {
        ...
        $msg2 = $admin->create_customer($address, $phone, $email, $account_type, $currency);

        $message = $msg1 . ' | ' . $msg2;

        $_SESSION['last_customer_id'] = $admin->last_customer_id;
    ...
    if (isset($_POST['update_customer'])) {
        ...
        $message = $admin->update_customer($cid, $address, $phone, $email);
    }
    ...
    if (isset($_POST['delete_customer'])) {
        ...
        $message     = $admin->delete_customer($passport_id, $login_id);
    }
    ...
    // preserve ?page=customers if used in your admin router
    if (isset($_GET['page'])) {
...
$q = $_GET['q'] ?? '';
if ($q !== '') {
    $rows = $admin->search_customer($q);
} else {
    $rows = $admin->read_all_customer();
}
?>
