...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['create_deposit'])) {
        ...
        $message = $admin->create_saving_deposit(
    ...
    if (isset($_POST['delete_deposit'])) {
        $id      = $_POST['deposit_id'] ?? '';
        $message = $admin->delete_saving_deposit($id);
    }
    ...
    if (isset($_POST['withdraw_deposit'])) {
        $id       = $_POST['deposit_id'] ?? '';
        $card_num = $_POST['card_num'] ?? '';
        $message  = $admin->withdraw_saving_deposit($id, $card_num);
    }
    ...
    // preserve ?page=deposits if used in your admin router
    if (isset($_GET['page'])) {
...
$q = $_GET['q'] ?? '';
if ($q !== '') {
    $rows = $admin->search_saving_deposit($q);
} else {
    $rows = $admin->read_all_saving_deposit();
}
...
<table>
    ...
    <tr>
        ...
        <!-- DELETE on separate column -->
        <td>
            <form method="post" style="display:inline;">
                <input type="hidden" name="deposit_id" value="<?= htmlspecialchars($d['deposit_id']) ?>">
                <button type="submit" name="delete_deposit" onclick="return confirm('Delete this saving deposit?');">
                    Delete
                </button>
            </form>
        </td>
